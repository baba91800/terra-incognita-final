...
import type { Badge, Monument, CountryDiscovery, DiscoveryLog } from '../types/game'
...
export default function ProfileScreen({ onClose, onReset, score, xp, level, levelTitle, totalTiles, totalDist, badges, monuments, countries, log, territory, tiles, playerLat, playerLng, t }: Props) {
  ...
  return (
    <>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {(['profile', 'territory', 'stats'] as const).map(tab_ => (
              <button key={tab_} onClick={() => setTab(tab_)} style={{
                ...
                {tab_ === 'profile' ? 'Profil' : tab_ === 'territory' ? 'Territoire' : 'Stats'}
              </button>
            ))}
          </div>
          ...
          {tab === 'profile' && (
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                <div onClick={() => setShowAvatarEditor(true)} style={{ width: 86, height: 86, borderRadius: '50%', background: 'rgba(0,245,212,0.1)', border: '2.5px solid rgba(0,245,212,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 42, boxShadow: '0 0 24px rgba(0,245,212,0.2)', cursor: 'pointer', overflow: 'hidden', position: 'relative' }}>
                  {avatarPhoto ? <img src={avatarPhoto} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="avatar" /> : avatar}
                </div>
                <div style={{ fontSize: 8, color: 'rgba(0,245,212,0.4)', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer' }} onClick={() => setShowAvatarEditor(true)}>Modifier l'avatar</div>
                ...
                {streak > 0 && <div style={{ fontSize: 11, color: 'rgba(255,165,0,0.8)', fontFamily: 'monospace' }}>🔥 {streak} jour{streak > 1 ? 's' : ''} consécutif{streak > 1 ? 's' : ''}</div>}
              </div>
              ...
              {/* Graphique activité */}
              <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 10, padding: '12px', marginBottom: 16 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                  {badges.map(b => (
                    <div key={b.id} title={b.name + ' — ' + b.description} style={{ width: 44, height: 44, borderRadius: 10, background: b.earned ? 'rgba(0,245,212,0.1)' : 'rgba(255,255,255,0.03)', border: `1px solid ${b.earned ? 'rgba(0,245,212,0.3)' : 'rgba(255,255,255,0.06)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, opacity: b.earned ? 1 : 0.2 }}>{b.icon}</div>
                  ))}
                </div>
              </div>
              ...
              {countries.length > 0 && (
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {countries.map(c => <span key={c.code} title={c.name} style={{ fontSize: 24, filter: `drop-shadow(0 0 4px ${RARITY_COLORS[c.rarity]}60)` }}>{c.flag}</span>)}
                  </div>
                </div>
              )}

              {/* Export / Import / Share */}
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 16, marginBottom: 8 }}>
                <div style={{ fontSize: 9, letterSpacing: '0.15em', color: 'rgba(255,255,255,0.2)', textTransform: 'uppercase', marginBottom: 10 }}>Données & Partage</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 10 }}>
                  <button onClick={() => exportData()} style={{ padding: '10px 6px', borderRadius: 8, cursor: 'pointer', background: 'rgba(0,245,212,0.07)', border: '1px solid rgba(0,245,212,0.2)', color: '#00f5d4', fontSize: 10, fontFamily: 'monospace', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    <span style={{ fontSize: 18 }}>📤</span>Export
                  </button>
                  <button onClick={() => importRef.current?.click()} style={{ padding: '10px 6px', borderRadius: 8, cursor: 'pointer', background: 'rgba(59,130,246,0.07)', border: '1px solid rgba(59,130,246,0.2)', color: '#3b82f6', fontSize: 10, fontFamily: 'monospace', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    <span style={{ fontSize: 18 }}>📥</span>Import
                  </button>
                  <button onClick={() => setShowShare(true)} style={{ padding: '10px 6px', borderRadius: 8, cursor: 'pointer', background: 'rgba(168,85,247,0.07)', border: '1px solid rgba(168,85,247,0.2)', color: '#a855f7', fontSize: 10, fontFamily: 'monospace', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    <span style={{ fontSize: 18 }}>🔗</span>Partager
                  </button>
                </div>
                <input ref={importRef} type="file" accept=".json" style={{ display: 'none' }} onChange={handleImport} />
                {importMsg && (
                  <div style={{ padding: '8px 12px', borderRadius: 8, background: importMsg.ok ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)', border: `1px solid ${importMsg.ok ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)'}`, fontSize: 11, color: importMsg.ok ? '#4ade80' : 'rgba(239,68,68,0.8)', marginBottom: 8 }}>
                    {importMsg.text}
                  </div>
                )}
              </div>
          ...
          {tab === 'territory' && (
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 20 }}>
                <button onClick={() => setShowGlobe(true)} style={{ padding: '14px', borderRadius: 12, cursor: 'pointer', background: 'rgba(0,245,212,0.08)', border: '1px solid rgba(0,245,212,0.2)', color: '#00f5d4', fontFamily: 'monospace', fontSize: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontSize: 28 }}>🌍</span><span>Vue Globe 3D</span>
                </button>
                <button onClick={() => setShowBoundary(true)} style={{ padding: '14px', borderRadius: 12, cursor: 'pointer', background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.2)', color: '#3b82f6', fontFamily: 'monospace', fontSize: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontSize: 28 }}>🏙️</span><span>Frontières ville</span>
                </button>
              </div>
              {[
          ...
          {tab === 'stats' && (
            <>
              <div style={{ fontSize: 9, letterSpacing: '0.15em', color: 'rgba(0,245,212,0.5)', textTransform: 'uppercase', marginBottom: 16 }}>
                Sites découverts — {discMonuments.length}
              </div>
      ...
      {showGlobe && <GlobeView playerLat={playerLat} playerLng={playerLng} tiles={tiles} countries={countries} onClose={() => setShowGlobe(false)} />}
      {showBoundary && <CityBoundary playerLat={playerLat} playerLng={playerLng} tiles={tiles} cityName={territory.city} onClose={() => setShowBoundary(false)} />}
      {showAvatarEditor && <AvatarEditor currentAvatar={avatar} currentPhoto={avatarPhoto} onAvatarChange={(a, p) => { setAvatar(a); setAvatarPhoto(p) }} onClose={() => setShowAvatarEditor(false)} />}
      {showShare && <ShareCard tiles={tiles} playerLat={playerLat} playerLng={playerLng} score={score} level={level} levelTitle={levelTitle} totalDist={totalDist} monuments={monuments} pseudo={pseudo} avatar={avatar} avatarPhoto={avatarPhoto} onClose={() => setShowShare(false)} />}
    </>
  )
}
