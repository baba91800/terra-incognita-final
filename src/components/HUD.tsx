...
import { hasNotificationPermission, hasAskedPermission, requestNotificationPermission, scheduleStreakReminder } from '../lib/notifications'
...
interface Props {
  ...
  gpsActive:boolean; gpsError:string|null; onStartGPS:()=>void; onStopGPS:()=>void
...
export default function HUD(p:Props) {
  ...
  // Proposer les notifications si pas encore demandé et streak > 0
  useEffect(() => {
    if (!hasAskedPermission() && !hasNotificationPermission() && streak > 0) {
  ...
  return (
    <>
            ...
            <div className="hud-panel" style={{padding:'8px 12px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:5}}>
                <div style={{display:'flex',alignItems:'center',gap:7}}>
                  <div style={{width:24,height:24,borderRadius:6,background:'linear-gradient(135deg,rgba(0,180,160,0.3),rgba(0,245,212,0.15))',border:'1px solid rgba(0,245,212,0.4)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:'bold',color:'#00f5d4',fontFamily:'monospace'}}>{p.level}</div>
              ...
              <div style={{height:5,background:'rgba(255,255,255,0.06)',borderRadius:3,overflow:'hidden'}}>
                <div style={{width:`${xpPct}%`,height:'100%',borderRadius:3,background:'linear-gradient(90deg,#00b4a0,#00f5d4)',boxShadow:'0 0 8px rgba(0,245,212,0.6)',transition:'width 0.6s ease'}} />
              </div>
            </div>
              ...
              {showLang && (
                <div style={{position:'absolute',right:0,top:'calc(100% + 6px)',background:'rgba(5,12,24,0.97)',border:'1px solid rgba(0,245,212,0.2)',borderRadius:10,overflow:'hidden',zIndex:800,minWidth:140,boxShadow:'0 8px 32px rgba(0,0,0,0.6)'}}>
                  {[{code:'fr',label:'Français',flag:'🇫🇷'},{code:'en',label:'English',flag:'🇬🇧'},{code:'es',label:'Español',flag:'🇪🇸'},{code:'de',label:'Deutsch',flag:'🇩🇪'},{code:'pt',label:'Português',flag:'🇵🇹'}].map(l=>(
                    <button key={l.code} onClick={()=>{p.onChangeLang(l.code as Lang);setShowLang(false)}} style={{display:'flex',alignItems:'center',gap:10,width:'100%',padding:'9px 14px',background:p.lang===l.code?'rgba(0,245,212,0.12)':'transparent',border:'none',color:'#fff',cursor:'pointer',fontSize:12,borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
      ...
      {/* ── GPS ERROR ── */}
      {p.gpsError && (
        <div style={{position:'absolute',top:100,left:'50%',transform:'translateX(-50%)',zIndex:700,background:'rgba(239,68,68,0.15)',border:'1px solid rgba(239,68,68,0.4)',borderRadius:10,padding:'10px 16px',maxWidth:280,textAlign:'center',animation:'toastIn 0.3s ease-out'}}>
          <div style={{fontSize:11,color:'rgba(239,68,68,0.9)',fontFamily:'monospace'}}>{p.gpsError}</div>
        </div>
      )}
      ...
      {panel==='badges'&&(
        <Panel title={`${t.badgesTitle} — ${earnedB.length}/${p.badges.length}`} left onClose={()=>setPanel('none')}>
          {p.badges.map(b=>(
            <div key={b.id} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 10px',borderRadius:8,border:`1px solid ${b.earned?'rgba(0,245,212,0.25)':'rgba(255,255,255,0.05)'}`,background:b.earned?'rgba(0,245,212,0.05)':'transparent',opacity:b.earned?1:0.4}}>
      ...
      {/* ── GPS CTA (Onboarding GPS amélioré) ── */}
      <div style={{position:'absolute',bottom:16,left:'50%',transform:'translateX(-50%)',zIndex:600,pointerEvents:'auto'}}>
        {!p.gpsActive ? (
          <button onClick={p.onStartGPS} style={{display:'flex',alignItems:'center',gap:14,cursor:'pointer',background:'rgba(5,12,24,0.94)',border:'1px solid rgba(0,245,212,0.35)',borderRadius:14,padding:'12px 22px',boxShadow:'0 0 30px rgba(0,0,0,0.5), 0 0 20px rgba(0,245,212,0.1)',transition:'all 0.2s'}}
            onMouseEnter={e=>{e.currentTarget.style.borderColor='rgba(0,245,212,0.6)';e.currentTarget.style.boxShadow='0 0 30px rgba(0,0,0,0.5), 0 0 30px rgba(0,245,212,0.2)'}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor='rgba(0,245,212,0.35)';e.currentTarget.style.boxShadow='0 0 30px rgba(0,0,0,0.5), 0 0 20px rgba(0,245,212,0.1)'}}
          >
            <div style={{width:40,height:40,borderRadius:'50%',background:'rgba(0,245,212,0.1)',border:'1px solid rgba(0,245,212,0.3)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:22}} className="animate-pulse">📍</div>
            <div style={{textAlign:'left'}}>
              <div style={{fontSize:14,fontWeight:'bold',color:'#00f5d4',letterSpacing:'0.05em'}}>{t.activateGPS}</div>
              <div style={{fontSize:10,color:'rgba(255,255,255,0.3)',marginTop:3}}>{t.activateGPSDesc}</div>
              <div style={{fontSize:9,color:'rgba(0,245,212,0.4)',marginTop:2}}>Appuie pour commencer →</div>
            </div>
          </button>
        ) : (
      ...
      {showNotifPrompt && (
        <div style={{position:'absolute',top:110,left:'50%',transform:'translateX(-50%)',zIndex:700,background:'rgba(5,12,24,0.97)',border:'1px solid rgba(255,165,0,0.3)',borderRadius:12,padding:'14px 18px',maxWidth:280,animation:'toastIn 0.4s ease-out',boxShadow:'0 0 30px rgba(0,0,0,0.6)'}}>
          <div style={{display:'flex',gap:10,alignItems:'flex-start'}}>
            <span style={{fontSize:22,flexShrink:0}}>🔔</span>
            <div>
              <div style={{fontSize:12,fontWeight:'bold',color:'#fff',marginBottom:4}}>Rappels de streak</div>
              <div style={{fontSize:10,color:'rgba(255,255,255,0.4)',marginBottom:10,lineHeight:1.5}}>Tu as une série de {streak} jours ! Active les notifications pour ne pas la perdre.</div>
              <div style={{display:'flex',gap:8}}>
                <button onClick={handleEnableNotifs} style={{flex:1,padding:'7px',borderRadius:7,cursor:'pointer',background:'rgba(255,165,0,0.15)',border:'1px solid rgba(255,165,0,0.4)',color:'rgba(255,165,0,0.9)',fontSize:11,fontFamily:'monospace'}}>Activer</button>
                <button onClick={()=>setShowNotifPrompt(false)} style={{flex:1,padding:'7px',borderRadius:7,cursor:'pointer',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.3)',fontSize:11}}>Plus tard</button>
              </div>
            </div>
          </div>
        </div>
...
function Panel({title,children,left,onClose}:{title:string;children:React.ReactNode;left?:boolean;onClose:()=>void}) {
  return (
    <div style={{position:'absolute',top:180,[left?'left':'right']:12,zIndex:600,width:280,background:'rgba(5,12,24,0.94)',border:'1px solid rgba(0,245,212,0.14)',borderRadius:14,padding:'12px 14px',backdropFilter:'blur(20px)',WebkitBackdropFilter:'blur(20px)',maxHeight:'58vh',overflowY:'auto',pointerEvents:'auto',boxShadow:'0 8px 40px rgba(0,0,0,0.6)'}}>
