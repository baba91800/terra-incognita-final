...
export default function HUD(p:Props) {
  ...
  const NAV_BTNS = [
    { id:'badges'    as Panel, icon:'🏅', count:earnedB.length },
    ...
    <>
      ...
      <div style={{position:'absolute',top:0,left:0,right:0,zIndex:600,pointerEvents:'none'}}>
        <div style={{display:'flex',alignItems:'stretch',justifyContent:'space-between',padding:'10px 12px',gap:10}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:8,pointerEvents:'auto',minWidth:160}}>
            <div className="hud-panel">
              ...
              <div style={{display:'flex',gap:14,marginTop:8}}>
                {[['km²',(p.totalTiles*0.0001).toFixed(3)],['Dist.',(p.totalDist/1000).toFixed(1)+' km'],['Sites',p.monuments.filter(m=>m.discovered).length.toString()]].map(([l,v])=>(
                  <div key={l as string}>
          ...
          {/* CENTER */}
          <div style={{flex:1,display:'flex',justifyContent:'center',alignItems:'flex-start',paddingTop:4,pointerEvents:'none'}}>
          ...
          {p.badges.map(b=>(
            <button
              ...
              style={{
                ...
                borderRadius:8,width:'100%',cursor:'pointer',textAlign:'left',
                border:`1px solid ${b.earned?'rgba(0,245,212,0.25)':'rgba(255,255,255,0.05)'}`,
                background:b.earned?'rgba(0,245,212,0.06)':'rgba(255,255,255,0.02)',
      ...
      {selectedBadge && (
        <div
          ...
          style={{position:'fixed',inset:0,zIndex:900,background:'rgba(2,5,15,0.88)',backdropFilter:'blur(10px)',display:'flex',alignItems:'center',justifyContent:'center',padding:24}}
        >
          <div
