...
export default function MiniMap({ tiles, playerLat, playerLng, path }: Props) {
  const ref = useRef<HTMLCanvasElement>(null)

  // FIX #15 — on sérialise tiles.size ET un hash sommaire pour détecter les vrais changements
  // tiles est un Set passé par référence dont la taille peut changer sans que l'objet change
  // On utilise tiles.size comme proxy suffisant ici (chaque nouvelle tuile incrémente la taille)
  useEffect(() => {
    const c = ref.current
    if (!c) return
    ...
    if (path.length > 1) {
      ...
      path.forEach((pt, i) => {
        const ptx = Math.floor(pt.lng * MPG / TILE_SIZE)
        const pty = Math.floor(pt.lat * MPL / TILE_SIZE)
        const x = ox + (ptx - minTx) * scale + scale / 2
        const y = oy + (maxTy - pty) * scale + scale / 2
  ...
  // FIX #15 — tiles.size comme dépendance est correct ici car chaque ajout de tuile
  // incrémente la taille. Le Set lui-même est stable par référence (useRef dans useGameEngine),
  // donc tiles.size est le seul signal de changement fiable.
  }, [tiles.size, playerLat, playerLng, path.length])
