    ...
    <div style={{position:'absolute',top:100,left:'50%',transform:'translateX(-50%)',zIndex:700,display:'flex',flexDirection:'column',gap:8,alignItems:'center',pointerEvents:'none',minWidth:240}}>
      {notifications.map(n => (
        <div key={n.id} className="toast hud-panel" style={{
          ...
          {n.type==='monument' && <ToastRow icon="🏆" label="Monument découvert" title={n.title} sub={n.rarity?`${RARITY_LABELS[n.rarity]} · +${n.points} XP`:undefined} color={n.rarity?RARITY_COLORS[n.rarity]:undefined}/>}
          {n.type==='badge'    && <ToastRow icon={n.icon||'🏅'} label="Badge débloqué" title={n.title} sub={n.subtitle} color="#00f5d4"/>}
          {n.type==='country'  && <ToastRow icon={n.icon||'🌍'} label="Nouveau pays !" title={n.title} sub={n.rarity?`${RARITY_LABELS[n.rarity]} · +${n.points} XP`:undefined} color={n.rarity?RARITY_COLORS[n.rarity]:undefined}/>}
          {n.type==='level'    && <ToastRow icon="🎖️" label="Level Up !" title={n.title} sub={n.subtitle} color="#f59e0b"/>}
          {n.type==='objective'&& <ToastRow icon="🎯" label="Objectif complété !" title={n.subtitle||n.title} sub={`+${n.points} XP`} color="#22c55e"/>}
        </div>
      ))}
    </div>
  )
...
function ToastRow({icon,label,title,sub,color}:{icon:string;label:string;title:string;sub?:string;color?:string}) {
  return (
