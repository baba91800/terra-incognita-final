...
import { dist, tilesInRadius } from '../lib/geo'
import {
  saveTiles, loadTiles, saveScore, loadScore, saveXP, loadXP,
  saveDist, loadDist, saveBadges, loadBadges, saveMonuments, loadMonuments,
  savePlayer, loadPlayer, saveCountries, loadCountries,
  saveObjectives, loadObjectives, saveLog, loadLog, savePath, loadPath
} from '../lib/storage'
import { fetchMonuments } from '../lib/overpass'
import { fetchTerritory, loadTerritory, type TerritoryData } from '../lib/territory'

// FIX #17 — Limite max de tuiles en localStorage (~50 000 = ~2MB)
const MAX_TILES_STORED = 50_000
...
export function useGameEngine() {
  ...
  // FIX #6 — état d'erreur GPS exposé
  ...
  // FIX #14 — streak déplacé hors de checkBadges, calculé une seule fois au démarrage
  const updateStreak = useCallback(() => {
    const today = new Date().toISOString().split('T')[0]
    const lastDay = localStorage.getItem('ti2_last_day') || ''
    if (lastDay === today) return // Déjà mis à jour aujourd'hui
  ...
  const revealAt = useCallback((lat: number, lng: number, ms: Monument[]) => {
    ...
    if (newK.length > 0) {
      ...
      applyXP(pts)
      // FIX #17 — sauvegarder seulement si on ne dépasse pas la limite
      if (tiles.current.size <= MAX_TILES_STORED) {
        saveTiles(tiles.current)
      } else if (tiles.current.size % 1000 === 0) {
        // Au-delà de la limite, on sauvegarde seulement par tranches de 1000
        // pour éviter de bloquer le thread principal trop souvent
        saveTiles(tiles.current)
      }
  ...
  // FIX #6 — gestion d'erreur GPS avec message lisible
      ...
      err => {
        ...
        switch (err.code) {
          ...
          case err.POSITION_UNAVAILABLE:
            setGpsError('Position GPS indisponible. Vérifiez votre signal.')
            break
          case err.TIMEOUT:
            setGpsError('GPS trop lent. Réessayez en extérieur.')
            break
          default:
  ...
  useEffect(() => {
    ...
    // FIX #14 — streak mis à jour au démarrage, pas à chaque badge check
    updateStreak()
  ...
  // FIX #8 — explorationPercent : 100% = 10 000 tuiles (1 km²)
  const explorationPercent = (Math.min(100, tiles.current.size / 10000) * 100).toFixed(1)
  ...
  return {
    ...
    gpsActive, gpsError, initialized, totalTiles: tiles.current.size,
...
// ── Heading gyroscopique ──────────────────────────────────────
export function useHeading() {
  const [heading, setHeading] = useState<number | null>(null)
  // FIX #13 — prevPos supprimé (était déclaré mais jamais utilisé)
