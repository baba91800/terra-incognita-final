...
export function useGameEngine() {
  ...
  const checkBadges=useCallback((ms:Monument[])=>{
    ...
    const checks=[
      {id:'b1',ok:tiles.current.size>=10},{id:'b10',ok:tiles.current.size>=200},
      {id:'b2',ok:tiles.current.size>=500},{id:'b3',ok:tiles.current.size>=2000},
      {id:'b4',ok:dm.length>=1},{id:'b5',ok:dm.length>=5},{id:'b6',ok:dm.length>=10},
      {id:'b7',ok:dm.some(m=>m.rarity==='legendary')},
      {id:'b8',ok:scoreR.current>=5000},{id:'b9',ok:dm.some(m=>m.rarity==='epic')},
    ]
    let changed=false
